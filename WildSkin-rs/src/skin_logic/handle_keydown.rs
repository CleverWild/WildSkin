...
const VK_5: i32 = 0x35; // '5' key (Ctrl+5 cycles gear variants), not F5 (0x74)

/// The raw-key half of `wndProc` (runs regardless of whether `ImGui` captured
/// the key). The original's F7 debug scratch hook is dropped, not translated.
///
/// # Safety
...
pub unsafe fn handle_keydown(vk_code: i32) {
    ...
    if vk_code == menu_key {
        ...
        if !now_open {
            ...
            state
                ...
                .lock()
                .unwrap()
                .save(&crate::app::config::config_dir(), model.as_deref());
        }
        return;
    }
    ...
    if !quick_skin_change {
        return; // F7 debug scratch hook dropped, see module doc.
    }
    ...
    if vk_code == next_key {
        ...
        if let Some(p_ref) = unsafe { off.player_ref() } {
            ...
            state
                ...
                .lock()
                .unwrap()
                .save(&crate::app::config::config_dir(), Some(&model));
        }
