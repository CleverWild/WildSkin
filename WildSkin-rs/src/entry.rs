...
fn hide_thread(thread: &winsafe::HTHREAD) -> bool {
    type NtSetInformationThreadFn = unsafe extern "system" fn(*mut std::ffi::c_void, u32, *const std::ffi::c_void, u32) -> i32;

    // ntdll is always loaded in every Windows process.
    let Ok(ntdll) = winsafe::HINSTANCE::GetModuleHandle(Some("ntdll.dll")) else { return false };
    let Ok(proc) = ntdll.GetProcAddress("NtSetInformationThread") else { return false };
...
pub fn attach(hmodule: HINSTANCE) {
    ...
    std::thread::spawn(move || {
        ...
        let Some(offsets) = (unsafe { crate::memory::resolve_all(base, game_client) }) else {
            let _ = winsafe::HWND::NULL.MessageBox("Failed to resolve one or more signatures.", shared::APP_NAME, winsafe::co::MB::OK);
            hudhook::eject();
            return;
        };
        std::thread::sleep(std::time::Duration::from_millis(500));

        let mut database = crate::skin_database::SkinDatabase::empty();
        ...
        // same live process.
        unsafe { database.load(champion_manager, &offsets); }
        ...
        let player_model = unsafe { offsets.player_ref() }.map(|p_ref| {
            ...
            // this same live process.
            let stack = unsafe { p_ref.character_data_stack_ref(offsets.fields.character_data_stack) };
            // SAFETY: `stack.base_skin.model` is a live, initialized field.
            unsafe { stack.base_skin.model.as_str() }.to_owned()
        });
        config.load(&crate::config::config_dir(), player_model.as_deref());

        crate::state::init(offsets, config, database);

        if let Err(e) = hudhook::Hudhook::builder()
            .with::<ImguiDx11Hooks>(crate::overlay::Overlay)
